...
//    }
    ...
    {
        $query = Article::find()->where(['is_published' => '1', 'lang_id' => Lang::getCurrent()->id])->orderBy('created DESC');
...
//        $model = Article::find()->where(['is_published' => 1, 'lang_id' => Lang::getCurrent()->id])->one();
    ...
    {
        if (($model = Article::find()->where(['slug' => $slug, 'lang_id' => Lang::getCurrent()->id])->one()) !== null) {
            return $model;
        } else {
